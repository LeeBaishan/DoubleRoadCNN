...
public class Tool {
    ...
    public static void setBackImgPad(double[][][] imgPad, double[][][] img, int p) {
        for (int i = 0; i < imgPad.length; i++) {
...
//        System.out.println("图片宽为：" + volume[0].length);
    ...
    public static double[][][] averageT(double[][][] pic1, double[][][]pic2, double[][][]pic3) {
        double[][][] temp = new double[pic1.length][pic1[0].length][pic1[0][0].length];
        for (int i = 0; i < temp.length; i++) {
            for (int j = 0; j < temp[0].length; j++) {
                for (int k = 0; k < temp[0][0].length; k++) {
                    temp[i][j][k] = (pic1[i][j][k] /(pic1[i][j][k] + pic2[i][j][k])) * pic3[i][j][k];
                }
            }
        }
        return temp;
    ...
    public static double[][][] getImage(double[][][] pic1, double[][][] pic2, double[][][]pic3, double[][][]pic4, double[][][]pic5) {
        double[][][] temp = new double[pic1.length][pic1[0].length][pic1[0][0].length];
        for (int i = 0; i < temp.length; i++) {
            for (int j = 0; j < temp[0].length; j++) {
                for (int k = 0; k < temp[0][0].length; k++) {
                    temp[i][j][k] = (pic1[i][j][k] + pic2[i][j][k] + pic3[i][j][k] + (pic4[i][j][k] * 0.00001) + (pic5[i][j][k] * 0.00001));
                }
            }
        }
        return temp;
    ...
    public static int getArraysMax(double[] out) {
        double max = 0;
        int index = 0;
